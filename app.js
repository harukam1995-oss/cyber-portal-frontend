(function(){
  ...
  var _payablesModuleLoad = null;
  function loadPayablesModule(){
    if (_payablesModuleLoad) return _payablesModuleLoad;
    _payablesModuleLoad = new Promise(function(resolve, reject){
      var s = document.createElement("script");
      s.src = "app.payables.js";
      s.async = true;
      s.onload = function(){
        if (window.__CP && typeof window.__CP.initPayables === "function") resolve();
        else { _payablesModuleLoad = null; reject(new Error("app.payables.js は読めたが initPayables 未登録")); }
      };
      s.onerror = function(){ _payablesModuleLoad = null; reject(new Error("app.payables.js の取得に失敗")); };
      document.head.appendChild(s);
    });
    return _payablesModuleLoad;
  }
  ...
  /* ================= 分離モジュールへのブリッジ =================
     請求書管理セクション(#view-payables・約1,340行)は app.payables.js に分離した。
     そのタブを初回に開いたとき loadPayablesModule() が <script> を注入し、
     app.payables.js は下の window.__CP からヘルパーを受け取って動く。
     HOME/メール/カレンダー等しか使わないロードではこの JS は取得も解析もされない。 */
  window.__CP = {
